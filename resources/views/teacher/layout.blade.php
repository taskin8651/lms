    ...
    <aside class="w-64 bg-white border-r shadow-sm hidden md:flex md:flex-col">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1 text-sm">
            ...
            <a href="{{ route('teacher.dashboard') }}"
               ...
               {{ request()->routeIs('teacher.dashboard') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-blue-50' }}">
                🏠 Dashboard
            </a>

            <a href="{{ route('teacher.batches') }}"
               class="px-4 py-2 rounded flex items-center gap-2
               {{ request()->routeIs('teacher.batches*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-blue-50' }}">
                📚 My Batches
            </a>

            <a href="{{ route('teacher.attendance') }}"
               class="px-4 py-2 rounded flex items-center gap-2
               {{ request()->routeIs('teacher.attendance*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-blue-50' }}">
                📝 Attendance
            </a>
