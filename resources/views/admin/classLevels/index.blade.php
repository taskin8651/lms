...
@can('class_level_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.class-levels.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.classLevel.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('global.app_csvImport') }}
            </button>
            @include('csvImport.modal', ['model' => 'ClassLevel', 'route' => 'admin.class-levels.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
                        ...
                        <tr data-entry-id="{{ $classLevel->id }}">
                            ...
                            <td>
                                @can('class_level_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.class-levels.show', $classLevel->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('class_level_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.class-levels.edit', $classLevel->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('class_level_delete')
                                    <form action="{{ route('admin.class-levels.destroy', $classLevel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('class_level_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.class-levels.massDestroy') }}",
