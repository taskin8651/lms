    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tests.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('test_type') ? 'is-invalid' : '' }}" name="test_type" id="test_type">
                    ...
                    @foreach(App\Models\Test::TEST_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('test_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_marks') ? 'is-invalid' : '' }}" type="text" name="total_marks" id="total_marks" value="{{ old('total_marks', '') }}">
