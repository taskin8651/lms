    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.subjects.update", [$subject->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="class_level_id">{{ trans('cruds.subject.fields.class_level') }}</label>
                <select class="form-control select2 {{ $errors->has('class_level') ? 'is-invalid' : '' }}" name="class_level_id" id="class_level_id">
                    @foreach($class_levels as $id => $entry)
                        <option value="{{ $id }}" {{ (old('class_level_id') ? old('class_level_id') : $subject->class_level->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.subject.fields.class_level_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="image">{{ trans('cruds.subject.fields.image') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.subject.fields.image_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    url: '{{ route('admin.subjects.storeMedia') }}',
    maxFilesize: 20, // MB
...
@if(isset($subject) && $subject->image)
      var file = {!! json_encode($subject->image) !!}
